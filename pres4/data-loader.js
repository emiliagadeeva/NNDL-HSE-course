...
class DataLoader {
    ...
    prepareSequences(storeIds, windowSize, trainSplit = 0.7) {
        ...
        storeIds.forEach(storeId => {
            ...
            const valEnd = trainEnd + Math.floor(totalSequences * 0.15); // 15% для валидации
            
            // Обучающая выборка (самые ранние данные)
            ...
            // Валидационная выборка (следующие данные)
            ...
            // Тестовая выборка (самые поздние данные)
            for (let i = valEnd; i < totalSequences; i++) {
                const sequence = this.createSequence(storeData, i, windowSize);
                const target = this.createTarget(storeData, i, windowSize);
        ...
        console.log(`Generated sequences: Train=${trainSequences.length}, Val=${valSequences.length}, Test=${testSequences.length}`);
